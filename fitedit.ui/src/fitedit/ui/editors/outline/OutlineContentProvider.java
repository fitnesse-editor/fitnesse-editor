...
public class OutlineContentProvider implements ITreeContentProvider {
    ...
    private void addAnnotations(String src) {
        ...
        for (SegmentTree seg : tree.children) {
            if (seg.nodeType != NodeType.FOLDING)
                continue;
            if (seg.matched.length() <= 1)
                continue;
            ...
            model.addAnnotation(new ProjectionAnnotation(), new Position(seg.position.offset, (startIdx
                    - seg.position.offset + seg.matched.length())));
        }
    }

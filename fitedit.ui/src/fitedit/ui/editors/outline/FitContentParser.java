package fitedit.ui.editors.outline;
...
import fitedit.ui.editors.outline.SegmentTree.NodeType;
...
public class FitContentParser {
    ...
    public SegmentTree parse(String src) {
        ...
        while (matcher.find()) {
            root.children.add(new SegmentTree(root, NodeType.FOLDING, matcher.group(1).trim(), matcher.group(2).trim(),
                    new Position(matcher.start())));
        }

        matcher = patternHeadline.matcher(src);
        while (matcher.find()) {
            root.children.add(new SegmentTree(root, NodeType.HEADLINE, matcher.group(1).trim(),
                    matcher.group(2).trim(), new Position(matcher.start())));
        }

        matcher = patternTable.matcher(src);
        while (matcher.find()) {
            root.children.add(new SegmentTree(root, NodeType.TABLE, "!|", matcher.group(1).trim(), new Position(matcher
                    .start())));
        }
